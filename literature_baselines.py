#!/usr/bin/env python3
"""
Hand-crafted (GLCM + vegetation-index + spectral) literature baselines, evaluated
with the SAME protocol as the DINO/ResNet50 pipeline:
...
def ger_items():
    df = pd.read_csv(os.path.join(ROOT, "datasets", "german_dino.csv"))
    items, splits = [], []
...
def run_pakistan():
    ...
    for name, (cls, grid) in MODELS.items():
        ...
        out[name]["y_true"] = y.tolist()
        out[name]["y_pred"] = preds.tolist()
...
def run_german():
    ...
    for name, (cls, grid) in MODELS.items():
        ...
        out[name]["y_true"] = y[te].tolist()
        out[name]["y_pred"] = pred.tolist()
        print(f"  GER {name:5s}: %RMSE={out[name]['percent_rmse']:.2f} R2={out[name]['r2']:.2f}")
    return out


def main():
    outdir = os.path.join(ROOT, "results"); os.makedirs(outdir, exist_ok=True)
    print("=== literature baselines: Germany (85/15) ===")
    ger = run_german()
    print("=== literature baselines: Pakistan (nested LOO) ===")
    pak = run_pakistan()
