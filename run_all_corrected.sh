#!/usr/bin/env bash
# Full corrected, extensively-tuned pipeline. Each step is independent and logged;
# a failure in one step does NOT abort the rest (so the screen session always finishes).
# Launch detached:  screen -dmS biomass bash run_all_corrected.sh
set -u
cd "$(dirname "$0")"
...
LOG=results_corrected/logs
mkdir -p "$LOG"
MASTER="$LOG/run_$(date +%Y%m%d_%H%M%S).log"
say(){ echo "[$(date +%H:%M:%S)] $*" | tee -a "$MASTER"; }

say "START corrected pipeline (rich hyper-parameter tuning)"
...
for combo in "pakistan dino" "pakistan resnet50" "german dino" "german resnet50"; do
  ...
  ( python corrected_pipeline.py --site "$site" --features "$feat" \
...
( python corrected_ablations.py  > "$LOG/ablations.log" 2>&1 ; echo "exit=$? ablations" >> "$MASTER" ) &
a=$!
( python corrected_literature.py > "$LOG/literature.log" 2>&1 ; echo "exit=$? literature" >> "$MASTER" ) &
b=$!
wait $a; wait $b
say "STEP 2 done"
...
# ── Step 4: regenerate all figures from the corrected JSONs ──────────────────
say "STEP 4: figures"
python make_corrected_figures.py > "$LOG/figures.log" 2>&1
echo "exit=$? figures" >> "$MASTER"
say "STEP 4 done"

# ── Step 5: refresh the human-readable results summary ───────────────────────
say "STEP 5: results summary"
python summarize_corrected.py > "$LOG/summary.log" 2>&1
echo "exit=$? summary" >> "$MASTER"

say "ALL DONE"
touch results_corrected/.RUN_COMPLETE
