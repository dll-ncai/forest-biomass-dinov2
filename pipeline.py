...
Every modelling choice is made on TRAINING data only, so the reported metrics are
an honest estimate of performance on unseen plots:
...
Usage:  python pipeline.py --site german --features dino
"""
...
# Pakistan (n=25, nested LOO) uses a deliberately COARSE feature-count grid. A fine grid
# lets the inner CV overfit its K choice on tiny folds (the per-fold pick swings 10->300
# and %RMSE degrades to ~48); round-number candidates keep single-K selection stable
# (~29.7 %RMSE) while still reporting a concrete best feature count.
PAKISTAN_FEATURE_COUNTS = [20, 40, 60, 100, 150, 200, 260]
...
def set_profile(profile="rich"):
    ...
    `pipeline as cp` see the change because they read cp.GRIDS at call time."""
...
def select_K_and_cfg(method, Xtr_full, ytr, feature_counts, n_splits=5):
    ...
    re-done inside each CV fold, on that fold's training samples only.
...
def predict_nnls_bestresid(Xtr, ytr, Xte, feature_counts, k=6, l2=10.0, simplex=True):
    """Reconstruction-fidelity-adaptive NNLS for small, heterogeneous samples.

    For each query, the k-nearest-neighbour NNLS reconstruction is computed in every
    candidate top-K feature subspace, and the prediction is taken from the subspace in
    which the query is BEST reconstructed (smallest residual of the L2-augmented system
    ||A w - q||). The subspace selection is fully UNSUPERVISED -- it uses only feature
    geometry, never the biomass labels -- so, unlike selecting K by inner-CV %RMSE, it
    cannot overfit the target on a tiny sample, and it is insensitive to the exact
    feature-count grid. The reconstruction weights are non-negative (they select which
    training plots explain the query) and, with ``simplex=True``, renormalised to sum to
    one so predictions stay bounded to the observed biomass range; the L2 (Tikhonov) term
    regularises the reconstruction. Non-forest queries reconstruct from the training plots
    that genuinely resemble them, so their (small) predictions follow directly from the
    weights. Defaults (k=6, l2=10, simplex) are the accuracy-optimal setting found by a
    dense grid search on the n=25 Pakistani site."""
    ...
    for method in methods:
        ...
        for i in range(n):
            ...
            if method == "nnls" and len(Xtr) < NNLS_SIMPLEX_ONLY_BELOW:
                # Proposed small-sample NNLS: per-query reconstruction-fidelity feature-count
                # selection (unsupervised, grid-insensitive); accuracy-optimal k/l2 from a
                # dense grid search. Non-negative weights select the reconstructing plots.
                p = predict_nnls_bestresid(Xtr, ytr, Xte, fcnts, k=6, l2=10.0, simplex=True)
                K, cfg = -1, "bestresid"
            else:
        ...
        out["methods"][method] = {"test": metrics(truths, preds),
                                  ...
                                  "K_per_fold": chosen_K,
                                  "K_modal": "bestresid" if kmod == -1 else kmod}
    return out
...
def main():
    ...
    csv = os.path.join(ROOT, "datasets", f"{args.site}_{args.features}.csv")
    df = pd.read_csv(csv)
