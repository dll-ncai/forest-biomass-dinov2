#!/usr/bin/env python3
"""
Regenerate every RESULTS figure from the corrected (leak-free, extensively-tuned)
experiments in results_corrected/.  Figures are written to figures_corrected/ as
PNG (300 dpi) + SVG + PDF, and the headline ones are copied into paper_writeup/.

Reads only the committed corrected JSONs, so it re-runs in seconds. Any figure whose
...
Colour = identity of the feature backbone, fixed order, Okabe-Ito (colour-blind safe):
    DINOv2 = blue,  ResNet50 = orange,  Literature(GLCM) = bluish-green.
"""
...
RES = os.path.join(ROOT, "results_corrected")
OUT = os.path.join(ROOT, "figures_corrected")
...
def fig_performance():
    ...
    for ax, (title, dino, resnet, litd) in zip(axes, sites):
        ...
        b1 = ax.bar(x - w/2, dv, w, color=C["dino"], label="DINOv2 (384-d)")
        ...
        handles = [b1, b2]
    fig.legend(handles, ["DINOv2 (384-d)", "ResNet50 (2048-d)"],
               loc="upper center", ncol=2, fontsize=9.5, bbox_to_anchor=(0.5, 0.99))
    fig.suptitle("Corrected (leak-free) biomass regression — %RMSE by backbone & model",
                 fontsize=12, y=1.06)
    save(fig, "fig_corrected_performance", also_paper=True)
...
def fig_scatter():
    ...
    if dp:
        ...
        r = dp["methods"][bn]
        panels.append(("Pakistan — DINOv2 + %s" % bn.upper(), r["y_true"], r["y_pred"],
                       r["test"], "Biomass (t/ha)"))
    dg = load("german_dino.json")
    if dg:
        bn, _ = best_of({m: rr["combined_test"] for m, rr in dg["methods"].items()})
        r = dg["methods"][bn]
        panels.append(("Germany — DINOv2 + %s" % bn.upper(), r["pooled_true"], r["pooled_pred"],
    ...
    fig.suptitle("Corrected best-model predictions vs. observations", fontsize=12, y=1.02)
    save(fig, "fig_corrected_scatter", also_paper=True)


# ───────────────── FIG 3: German ablation (DINOv2) ─────────────────
def fig_ablation():
    ...
    ax.set_xticks(x); ax.set_xticklabels([mlabel[m] for m in methods])
    ax.set_ylabel("%RMSE  (lower is better)")
    ax.set_title("Germany ablation (DINOv2) — leak-free", fontsize=11)
    ax.legend(fontsize=8.5); ax.yaxis.grid(True, color=GRID, lw=0.7); ax.set_axisbelow(True)
    save(fig, "fig_corrected_ablation_german", also_paper=True)


# ───────────────── FIG 4: performance vs. #features (German DINOv2 CV curve) ─────
def fig_perf_vs_features():
    ...
    ax.set_title("Germany DINOv2 — train-only CV vs. feature count", fontsize=11)
    ax.grid(True, color=GRID, lw=0.7); ax.set_axisbelow(True)
    save(fig, "fig_corrected_perf_vs_features", also_paper=True)
...
def fig_robustness():
    ...
    if not d or "summary" not in d:
        return
    s = d["summary"]
    backbones = [("dino", "DINOv2", C["dino"]), ("resnet50", "ResNet50", C["resnet50"])]
    methods = ["nnls", "ridge", "xgb"]; mlabel = {"nnls": "NNLS", "ridge": "Ridge", "xgb": "XGBoost"}
    ...
    ax.legend(fontsize=8.5); ax.yaxis.grid(True, color=GRID, lw=0.7); ax.set_axisbelow(True)
    save(fig, "fig_corrected_robustness_german", also_paper=True)


def main():
    print("Generating corrected-results figures -> figures_corrected/")
