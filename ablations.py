#!/usr/bin/env python3
"""
Ablations for the German site, using the same train-only clustering / ranking /
K-selection protocol as the main pipeline:
...
Run for both feature backbones.  Results -> results/ablations_*.json
"""
import os, json
...
def load(site, feat):
    return pd.read_csv(os.path.join(ROOT, "datasets", f"{site}_{feat}.csv"))
